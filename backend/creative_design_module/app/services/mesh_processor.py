...
class MeshProcessor:
    ...
    def repair_mesh(self, mesh: trimesh.Trimesh) -> trimesh.Trimesh:
        ...
        mesh.merge_vertices()
        # 兼容不同 trimesh 版本：有些版本没有 remove_duplicate_faces
        try:
            if hasattr(mesh, "remove_duplicate_faces"):
                mesh.remove_duplicate_faces()
            else:
                unique_face_indices = trimesh.grouping.unique_rows(mesh.faces)[0]
                mesh.update_faces(unique_face_indices)
                mesh.remove_unreferenced_vertices()
        except Exception as e:
            logger.warning(f"  重复面清理跳过: {e}")
    ...
    def remesh_quad(
        ...
        # 方法 1: 直接使用 trimesh 的简化/细分策略，避免依赖 pymeshlab 的格式兼容问题
        try:
